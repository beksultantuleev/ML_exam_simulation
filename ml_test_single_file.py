...
{'Data should always be normalized or standardized before applying PCA, in order to remove the effect of outliers.':'f'},
...
{'The cosine similarity is not influenced by the units of measure used to express the input dimensions of data, because it depends only on the direction of the vectors.':'t'},
...
{'Models with too many parameters tend to fail because of a large bias, since they define models which might easily overfit data':'f'},
...
{'Consider a multilayer perceptron with bias units and 2 layers, where the first layer contains 3 perceptrons and the second layer contains 1 perceptron. With such a structure, the multilayer perceptron contains 2 bias units, and 4 outgoing arcs in total':'t'},
...
{'In logistic regression, a logistic function transforms the output of a linear model in the interval [0,1] so that it can be interpreted as a probability to be used for classifying the input. Points are classified as belonging to one of the classes if the predicted probability is greater or equal/lower than a certain threshold':'t'},
{'Consider a dataset composed by n examples (x_i,y_i ) with 1≤i≤n. The linear correlation coefficients between xi and yi may change if x values are centred by subtracting their mean value mu in the following way xi – mu':'f'},
{'If a supervised learning model has a lower error on the training set with respect to other model B, A should be preferred':'f'},
{'If only an external representation of the entities to be clustered is available, it is still possible to compute the internal representation of the entities':'f'},
{'Machine learning techniques can build effective models only if abundant data is available for training and testing because good models must be able to achieve zero error on the data used for training the model':'f'},
...
{'The models used to solve supervised learning problems are built by optimizing error functions which are not supposed to be smooth, because otherwise the generalisation of models on unseen data is not possible':'f'},
{'Nearest neighbours algorithms can be applied to both classification and regression problems. Unfortunately, they require more and more time to execute as the number of stored points and input features increase. In such a scenario, to reduce memory consumption and improve time efficiency, feature selection methods can be helpful tools which improve the efficiency of nearest neighbors algorithms without causing any loss of information':'f'},
{'Given a dataset of n examples (x_i,y_i ) with 1≤i≤n, let q be a point whose output needs to be predicted using Locally-Weighted Regression. The significance s_i can be defined as s_i=exp⁡(-〖|x_i-q||〗^2/W_K ) where W_K is some constant which controls the sensitivity of the model to points distant from x. As the distance between x_i and q decreases (with respect to W_K), the lower is the impact that x_i has on the prediction':'f'},
{'The normalization of data consists of rescaling all input dimensions so that they have zero-mean (centered around 0) and unit-variance (variance equal to 1)':'f'},
{'In logistic regression, the logistic function does not determine the threshold according to which the output of the linear model is classified':'t'},
{'Even if the Pearson correlation coefficient between two data features is zero, it is not possible to state that such features are independent':'t'},
{'In linear regression, if the number of input variables is 10 and one starts training from 40 different examples, the parameters of the linear model obtaining zero error on the examples can always be determined':'f'},
{'The objective of regression is to model the relationship between a real value and a set of input values, which must be numerical, as in the case of linear regression':'t'},
{'Given a dataset of mushrooms, where edible mushrooms belong to the positive class and deadly mushrooms belong to the negative class, maximizing recall is more important than maximizing precision':'f'},
...
{'Consider a multilayer perceptron with bias units and 2 layers, where the first layer contains 3 perceptrons and the second layer contains 1 perceptron. With such a structure the multilayer perceptron contains 2 bias units':'t'},
...
{'K-means clustering is not a supervised learning technique':'t'},
{'The quantization error measures the error obtained by substituting the entities of each cluster with the respective prototype. Therefore, a lower quantization error implies a better clustering':'f'},
...
{'In logistic regression the standard sigmoid function is defined as 1/(1+e^(-t) ) with t=w^T x, where w is a vector of weights and x is a vector of input data':'t'},
{'Given a dataset of n examples (x_i,y_i ) with 1≤i≤n. The linear correlation between x, and yi does not change if x values are normalized by dividing them by their standard deviation sigma >0 in the following manner xi/sigma':'t'},
{'The k-nearest neighbour algorithm is a lazy memory based technique which determines the output of a new data point x according to the outputs of the k data points which are nearest to x. In the case of classification, the output of a new input vector x can be obtained as the majority of the outputs of the knearest vectors in memory. If an erroneous classification might cause severe consequences, the algorithm can be changed to classify x only if all the k nearest neighbors have the same output label':'t'},
{'Standard mathematical optimization requires the existence of goodness function to be optimized, and the definition of these functions usually requires an expensive effort in the real world. Machine learning does not require an explicit definition of goodness functions, because it can automatically build goodness models. However, if the amount of available data is not sufficiently large, the generalization capability of these models might be unsatisfactory':'t'},
{'In locally weighted regression, given a dataset of n examples (x_i,y_i ) with 1≤i≤n, least squares methods are used to minimize the error ∑_(i=1)^n▒〖y_i 〖(w^T x_i-s_i)〗^2 〗, where s_i is the significance of x_i':'f'},
{'Wrapper methods consider different subsets of features of data to build a model, and they run training and testing for possible subsets of features in order to decide the best subset to be used. Although they are computationally intensive, they usually provide the best subset of features to be used for building a model':'t'},
{'A function f(x), where x is a vector, is linear if for every α and β and every couple of vectors x  y=f(αx+βy)=αf(x)+βf(y)':'t'},
{'The intersection of 2 convex sets is not always convex':'f'},
{'Consider a two-dimension space, where each point x = (x_1, x_2) and x∈R^2. The union of points satisfying x_1 < 0 and x_2 < 0 form a convex set': 'f'},
{'Stratified cross validation is designed to validate supervised learning models with hidden layers':'f'}
]
